@php
	...
	$price = $curr == 'vnd' ? number_format($package->package_price ?? 0, 0, ',', '.') : number_format($package->package_price ?? 0, 2, '.', ',');
...
<div class="paypackages">
    ...
    <div class="four-plan">
        ...
        <div class="row"> @foreach($packages as $package)
            <div class="col-md-4 col-sm-6 col-xs-12">
                <ul class="boxes">
                    ...
                    <li>
                        <div class="main-plan">
                            <div class="plan-price1-1">{{ $curr }}</div>
                            <div class="plan-price1-2">{{$price}}</div>
		...
		<li class="order paypal"><a href="{{route('order.package', $package->id)}}">{{__('pay with paypal')}}</a></li>
		@endif
		@if((bool)$siteSetting->is_stripe_active)
		<li class="order"><a href="{{route('stripe.order.form', [$package->id, 'new'])}}">{{__('pay with stripe')}}</a></li>
		@endif

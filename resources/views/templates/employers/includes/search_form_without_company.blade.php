<form action="{{route('job.list')}}" method="get">
    <div class="searchbar">
        ...
        <div class="col-md-{{((bool)$siteSetting->country_specific_site)? 6:3}}">
            {!! Form::select('functional_area_id[]', ['' => __('Lựa chọn Bộ phận chức năng')]+$functionalAreas, Request::get('functional_area_id', null), array('class'=>'form-control', 'id'=>'functional_area_id')) !!}
        </div>
        ...
        <div class="col-md-3">
            <span id="state_dd">
                {!! Form::select('state_id[]', ['' => __('Chọn vùng miền')], Request::get('state_id', null), array('class'=>'form-control', 'id'=>'state_id')) !!}
            </span>
        </div>
        <div class="col-md-3">
            <span id="city_dd">
		...
		<div class="input-group">
		  <input type="text"  name="search" id="jbsearch" value="{{Request::get('search', '')}}" class="form-control" placeholder="{{__('Nhập kỹ năng hoặc trí công việc')}}" autocomplete="off" />
		  <span class="input-group-btn">
			<input type="submit" class="btn" value="{{__('Tìm việc')}}">
		  </span>
		</div>
		</div>

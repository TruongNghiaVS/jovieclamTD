<div class="tab-pane fade show active" id="posted-job-1-pane" role="tabpanel" aria-labelledby="posted-job-1" tabindex="0">
            ...
            @if(isset($job_applications) && count($job_applications))
                @foreach($job_applications as $job_application)
                    ...
                    <div class="profile-footer">
                        <button type="button" class="btn btn-light btn-schedule-interview" data-bs-toggle="modal" data-bs-target="#modalScheduleInterview">
                            <a href="{{route('add.to.favourite.applicant',[$job_application->id, $user->id,$job->id, $company->id ])}}">{{ __('Add to shortlist')}}</a>
                        </button>
                        {{--

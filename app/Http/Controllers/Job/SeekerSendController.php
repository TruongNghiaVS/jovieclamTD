...
class SeekerSendController extends Controller
{
    ...
    public function all_messages()
    {
        ...
        return view(config('app.THEME_PATH').'.seeker.all-messages')->with($data);
    }
    public function append_messages(Request $request)
    {
        ...
        $search = view(config('app.THEME_PATH').".seeker.append-messages", compact('messages', 'seeker', 'company'))->render();
        return $search;
    }
    ...
    function submitnew_message(Request $request)
    {
        ...
        if(isset($request->new) && $request->new == true) {
            $messages = CompanyMessage::where('seeker_id', Auth::user()->id)
                        ->orderBy('id', 'asc')
                        ->groupBy(['company_id'])
                        ->pluck('company_id')
                        ->toArray();

            $data['companies'] = Company::whereIn('id', $messages)->get();
            return view(config('app.THEME_PATH').'.seeker.all-messages')->with($data);
        }
        ...
        if ($message->save() == true) {
            ...
            $company = Company::where('id', $request->company_id)->first();
            $search = view(config('app.THEME_PATH').".seeker.appendonly-messages", compact('messages', 'seeker', 'company'))->render();
            return $search;
        }
    }

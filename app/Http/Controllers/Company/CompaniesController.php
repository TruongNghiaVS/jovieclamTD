...
class CompaniesController extends Controller
{
    ...
    public function company_listing(Request $request)
    {
        ...
        if(!empty($params['search'])) {
            $query->where(function($q) use($params) {
                ...
                ->orWhere('ceo', 'like', '%'.$params['search'].'%')
    ...
    public function postedJobs(Request $request)
    {
        ...
        return view(config('app.THEME_PATH').'.job.company_posted_jobs')
                        ->with('jobs', $jobs);
    }
    ...
    public function refreshFrontJob($job_id)
    {
        ...
        if(!empty($job) && $job->is_featured==1 && $job->status == Job::POST_ACTIVE) {
            $job->updated_at = Carbon::now();
            $job->refresh_at = Carbon::now();
            $job->update();

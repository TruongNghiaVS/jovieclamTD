...
class JobShiftController extends Controller
{
    ...
    public function storeJobShift(JobShiftFormRequest $request)
    {
        ...
        $jobShift->update();
        flash(__('Job Shift has been added!'))->success();
    ...
    public function updateJobShift($id, JobShiftFormRequest $request)
    {
        ...
        $jobShift->update();
        flash('Ca kíp đã được cập nhật!')->success();
        return \Redirect::route('edit.job.shift', array($jobShift->id));
    }
        ...
        return Datatables::of($jobShifts)
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						<li>
							<a href="' . route('edit.job.shift', ['id' => $jobShifts->id]) . '"><i class="fa fa-pencil" aria-hidden="true"></i>Sửa</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="deleteJobShift(' . $jobShifts->id . ', ' . $jobShifts->is_default . ');" class=""><i class="fa fa-trash-o" aria-hidden="true"></i>Xóa</a>
						</li>
						<li>

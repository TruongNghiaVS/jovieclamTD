...
class AdminController extends Controller
{
    ...
    {
        return view(config('admin.admin.index');
    }
    ...
    public function createAdminUser()
    {
         ...
         */
        return view(config('admin.admin.create')->with('roles', $roles);
    }
    ...
    public function editAdminUser($id)
    {
        ...
        $roles = Role::select('role_name', 'id')->orderBy('role_name')->pluck('role_name', 'id')->toArray();
        return view(config('admin.admin.edit')->with('roles', $roles)->with('user', $user);
    }
    ...
    public function updateAdminUser($id, AdminFormRequest $request)
    {
        ...
        $user->role_id = $request->role_id;
        $user->save();
        flash('Quản trị viên đã được cập nhật!')->success();
        return \Redirect::route('edit.admin.user', array($user->id));
    }
    ...
    public function fetchAdminUsersData()
    {
        ...
        return Datatables::of($users)
                        ->addColumn('action', function ($user) {
                            return '<a href="' . route('edit.admin.user', ['id' => $user->id]) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i> Sửa</a><a href="javascript:void(0);" onclick="delete_user(' . $user->id . ');" class="btn btn-xs btn-danger"><i class="glyphicon glyphicon-remove"></i> Xóa</a>';
                        })

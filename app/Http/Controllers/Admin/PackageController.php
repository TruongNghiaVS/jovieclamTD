...
class PackageController extends Controller
{
    ...
    public function storePackage(PackageFormRequest $request)
    {
        ...
        $package->package_price = str_replace(",","",$request->input('package_price'));
        ...
        $currency = Country::where('lang', $language)->first()->currency ?? 'vnd';
        $package->currency = $currency;
        $package->save();
        /*         * ************************************ */
        flash(__('Package has been added!'))->success();
    ...
    public function editPackage($id)
    {
        ...
        $package->package_price =  \App::getLocale()== 'en' ? number_format($package->package_price, 2, '.', ',')
    ...
    public function updatePackage($id, PackageFormRequest $request)
    {
        ...
        $package->currency = $currency;
        $package->update();
        flash(__('Package has been updated!'))->success();
        return \Redirect::route('edit.package', array($package->id));
    }
        ...
        return Datatables::of($packages)
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						<li>
							<a href="' . route('edit.package', ['id' => $packages->id]) . '"><i class="fa fa-pencil" aria-hidden="true"></i>Sửa</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="deletePackage(' . $packages->id . ');" class=""><i class="fa fa-trash-o" aria-hidden="true"></i>Xóa</a>
						</li>
					</ul>
				</div>';
                        })

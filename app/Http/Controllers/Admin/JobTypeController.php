...
class JobTypeController extends Controller
{
    ...
    public function storeJobType(JobTypeFormRequest $request)
    {
        ...
        $jobType->update();
        flash(__('Job Type has been added!'))->success();
    ...
    public function updateJobType($id, JobTypeFormRequest $request)
    {
        ...
        $jobType->update();
        flash('Loại hình công việc đã được cập nhật!')->success();
        return \Redirect::route('edit.job.type', array($jobType->id));
    }
        ...
        return Datatables::of($jobTypes)
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						<li>
							<a href="' . route('edit.job.type', ['id' => $jobTypes->id]) . '"><i class="fa fa-pencil" aria-hidden="true"></i>Sửa</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="deleteJobType(' . $jobTypes->id . ', ' . $jobTypes->is_default . ');" class=""><i class="fa fa-trash-o" aria-hidden="true"></i>Xóa</a>
						</li>
						<li>

...
class LanguageLevelController extends Controller
{
    ...
    {
        $languages = DataArrayHelper::languagesNativeCodeArray();
        $languageLevels = config('app.LANGUAGE_RANKS');
        $languageLevel = LanguageLevel::findOrFail(3);
    ...
    public function storeLanguageLevel(LanguageLevelFormRequest $request)
    {
        ...
        /*         * ************************************ */
        flash(__('Language Level has been added!'))->success();
    ...
    public function updateLanguageLevel($id, LanguageLevelFormRequest $request)
    {
        ...
        /*         * ************************************ */
        $languageLevel->update();
        flash('Trình độ ngôn ngữ đã được cập nhật/ Language Level has been updated!')->success();
        return \Redirect::route('edit.language.level', array($languageLevel->id));
    }
        ...
        return Datatables::of($languageLevels)
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						<li>
							<a href="' . route('edit.language.level', ['id' => $languageLevels->id]) . '"><i class="fa fa-pencil" aria-hidden="true"></i>Sửa</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="deleteLanguageLevel(' . $languageLevels->id . ', ' . $languageLevels->is_default . ');" class=""><i class="fa fa-trash-o" aria-hidden="true"></i>Xóa</a>
						</li>
						<li>

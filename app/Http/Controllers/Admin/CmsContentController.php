...
class CmsContentController extends Controller
{
    ...
    public function storeCmsContent(CmsContentFormRequest $request)
    {
        ...
        $cmsContent->page_slug = $request->input('page_slug');
        $page_image = $request->file('page_image');
        if ($page_image != '') {
            $uploadPageImage = $this->upload_file($page_image);
            $cmsContent->page_image = $uploadPageImage['file_path'];
        }

        $cmsContent->save();
        flash(__('C.M.S page has been added!'))->success();
    ...
    public function updateCmsContent($id, CmsContentFormRequest $request)
    {
        ...
        $cmsContent->update();
        flash(__('C.M.S page has been updated!'))->success();
        return \Redirect::route('edit.cmsContent', array($cmsContent->id));
    }
        ...
        return Datatables::of($cmsContent)
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						<li>
							<a href="' . route('edit.cmsContent', ['id' => $cmsContent->id]) . '"><i class="fa fa-pencil" aria-hidden="true"></i>Sửa</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="delete_cmsContent(' . $cmsContent->id . ');" class=""><i class="fa fa-trash-o" aria-hidden="true"></i>Xóa</a>
						</li>																																							
					</ul>
				</div>';
                        })

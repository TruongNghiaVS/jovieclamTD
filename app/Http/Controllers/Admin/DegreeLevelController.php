...
class DegreeLevelController extends Controller
{
    ...
    public function storeDegreeLevel(DegreeLevelFormRequest $request)
    {
        ...
        $degreeLevel->update();
        /*         * ************************************ */
        flash(__('Degree Level has been added!'))->success();
    ...
    public function updateDegreeLevel($id, DegreeLevelFormRequest $request)
    {
        ...
        /*         * ************************************ */
        $degreeLevel->update();
        flash(__('Degree Level has been updated!'))->success();
        return \Redirect::route('edit.degree.level', array($degreeLevel->id));
    }
        ...
        return Datatables::of($degreeLevels)
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						<li>
							<a href="' . route('edit.degree.level', ['id' => $degreeLevels->id]) . '"><i class="fa fa-pencil" aria-hidden="true"></i>Sửa</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="deleteDegreeLevel(' . $degreeLevels->id . ', ' . $degreeLevels->is_default . ');" class=""><i class="fa fa-trash-o" aria-hidden="true"></i>Xóa</a>
						</li>
						<li>

...
class DegreeTypeController extends Controller
{
    ...
    public function storeDegreeType(DegreeTypeFormRequest $request)
    {
        ...
        $degreeType->update();
        /*         * ************************************ */
        flash(__('Degree Type has been added!'))->success();
    ...
    public function updateDegreeType($id, DegreeTypeFormRequest $request)
    {
        ...
        /*         * ************************************ */
        $degreeType->update();
        flash('Ngành học đã được cập nhật!')->success();
        return \Redirect::route('edit.degree.type', array($degreeType->id));
    }
        ...
        return Datatables::of($degreeTypes)
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						<li>
							<a href="' . route('edit.degree.type', ['id' => $degreeTypes->id]) . '"><i class="fa fa-pencil" aria-hidden="true"></i>Sửa</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="deleteDegreeType(' . $degreeTypes->id . ', ' . $degreeTypes->is_default . ');" class=""><i class="fa fa-trash-o" aria-hidden="true"></i>Xóa</a>
						</li>
						<li>

...
class OwnershipTypeController extends Controller
{
    ...
    public function storeOwnershipType(OwnershipTypeFormRequest $request)
    {
        ...
        /*         * ************************************ */
        flash(__('Ownership Type has been added!'))->success();
    ...
    public function updateOwnershipType($id, OwnershipTypeFormRequest $request)
    {
        ...
        /*         * ************************************ */
        $ownershipType->update();
        flash('Loại hình doanh nghiệp đã được cập nhật!')->success();
        return \Redirect::route('edit.ownership.type', array($ownershipType->id));
    }
        ...
        return Datatables::of($ownershipTypes)
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						<li>
							<a href="' . route('edit.ownership.type', ['id' => $ownershipTypes->id]) . '"><i class="fa fa-pencil" aria-hidden="true"></i>Sửa</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="deleteOwnershipType(' . $ownershipTypes->id . ', ' . $ownershipTypes->is_default . ');" class=""><i class="fa fa-trash-o" aria-hidden="true"></i>Xóa</a>
						</li>
						<li>

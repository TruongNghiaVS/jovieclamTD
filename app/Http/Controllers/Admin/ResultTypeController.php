...
class ResultTypeController extends Controller
{
    ...
    public function updateResultType($id, ResultTypeFormRequest $request)
    {
        ...
        /*         * ************************************ */
        $resultType->update();
        flash('Hình thức đánh giá được cập nhật!')->success();
        return \Redirect::route('edit.result.type', array($resultType->id));
    }
        ...
        return Datatables::of($resultTypes)
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						<li>
							<a href="' . route('edit.result.type', ['id' => $resultTypes->id]) . '"><i class="fa fa-pencil" aria-hidden="true"></i>Sửa</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="deleteResultType(' . $resultTypes->id . ', ' . $resultTypes->is_default . ');" class=""><i class="fa fa-trash-o" aria-hidden="true"></i>Xóa</a>
						</li>
						<li>

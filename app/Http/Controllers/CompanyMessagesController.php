...
class CompanyMessagesController extends Controller
{
    ...
    function submitnew_message_seeker(Request $request)
    {
        ...
        if ($message->save() == true) {
            $arr = array('msg' => __('Your message have successfully been posted. '), 'status' => true);
        }
    ...
    function submit_message(Request $request)
    {
        ...
        if ($message->save() == true) {
            ...
            $search = view(config('app.THEME_PATH').".company.appendonly-messages", compact('messages', 'seeker', 'company'))->render();
            return $search;
        }
    }
    public function all_messages()
    {
        ...
        return view(config('app.THEME_PATH').'.company.all-messages')->with($data);
    }
    public function append_messages(Request $request)
    {
        ...
        $seeker = User::where('id', $seeker_id)->first();
        $company = Company::where('id', $company_id)->first();
        $search = view(config('app.THEME_PATH').".company.append-messages", compact('messages', 'seeker', 'company'))->render();
        return $search;
    }

...
class DataArrayHelper
{
    ...
    public static function langLanguageLevelsArrayRep()
    {
        $array = LanguageLevel::select('language_levels.language_level', 'language_levels.language_level_id')->lang()->active()->sorted()->pluck('language_levels.language_level', 'language_levels.language_level_id')->toArray();
        if ((int) count($array) === 0) {
            $array = self::defaultLanguageLevelsArray();
    ...
    public static function langSalaryFromArray()
    {
        $cur = \App::getLocale()== 'vi' ? 'VND' : 'USD';
        \DB::statement("SET SQL_MODE=''");
        $array = Job::where('salary_currency', $cur)->groupBy('salary_from')->orderBy('salary_from', 'asc')->pluck('salary_from', 'id')->toArray();
        return array_map(function ($value) use ($cur) {
            if($value!=0) return number_format($value,0) . ' ' . $cur;
        }, $array);
    }

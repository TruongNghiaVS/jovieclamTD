...
trait ProfileCvsTrait
{
    ...
    public function showProfileCvs(Request $request, $user_id)
    {
        ...
        if (isset($user) && count($user->profileCvs)):
            ...
            foreach ($user->profileCvs as $cv):
                $default = 'Không mặc định/ Not Default';
                if ($cv->is_default == 1)
                    $default = 'Mặc định/ Default';
                $html .= '<div class="item " id="cv_' . $cv->id . '">
                    <div class="row">
						<div class="col-8 col-md-8">
                            <div class="con-skill">
                                <span class="sub-title">' . ImgUploader::get_doc("cvs/$cv->cv_file", $cv->title, $cv->title) . '<span class="tag-default">' . $default . '</span> </span>
                            </div>
                        </div>
						<div class="col-4 col-md-4">
                            <div class="con-action">
                                <a href="javascript:;" onclick="showProfileCvEditModal(' . $cv->id . ');" class="btn-action"><span class="iconmoon icon-edit-icon"></span></a>&nbsp;&nbsp;<a href="javascript:;" onclick="delete_profile_cv(' . $cv->id . ');" class="btn-action"><span class="iconmoon icon-trash"></span></a>
                            </div>
                        </div>
                        </div>
					</div>';
    ...
    {
        $user = User::find($user_id);
        $returnHTML = view('templates.vietstar.user.forms.cv.cv_modal')->with('user', $user)->render();

...
trait JobTrait
{
    ...
    private function assignJobValues($job, $request)
    {
        ...
        $job->salary_to = (int) $request->input('salary_to') ?? 0;
    ...
    public function storeJob(JobFormRequest $request)
    {
        ...
        return \Redirect::route('edit.job', array($job->id))->with('success', __('Job has been added'));
    }
    ...
    public function createFrontJob()
    {
        ...
        $jobSkillIds = array();
        return view('admin.job.add_edit_job')
                        ->with('countries', $countries)
                        ->with('currencies', array_unique($currencies))
                        ->with('careerLevels', $careerLevels)
                        ->with('functionalAreas', $functionalAreas)
